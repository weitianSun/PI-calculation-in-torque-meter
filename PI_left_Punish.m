%%%this script is used to calculate PI of flying trace when punishment is
%%%on Left-positive
function PILeftPunish = PI_left_Punish(time, angle);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% PI calculation in pretest
pretestrow = find(time>0 & time < 121); %%%% train1 phase from 0s to 120s%%%
pretest = time(pretestrow);%%%%%%%%%% time value in pretest phase%%%
pretestVal = angle(pretestrow);%%%%%%% trace value in pretest phase%%
tarow = find(pretestVal<0);  %%%%%%%%%% find which row that trace value<0, i.e when turning right%%%
RTP = pretest(tarow(2:end)); %%%%%%find at which time point, fly on right side,RTP = right time point%%%
preRightRow = tarow-1;
PRTP = pretest(preRightRow(2:end)); %%%%%%%%%the last time point before  RTP, PRTP = pre right time point%%%%
ta = sum(RTP-PRTP); %%%%%the total time when fly on the right side, i.e. non punishment time%%
...
train1Val = angle(train1row);%%%%%%% trace value in train phase%%
tarow = find(train1Val<0);  %%%%%%%%%% find which row that trace value<0, i.e when turning right%%%
RTP = train1(tarow(2:end)); %%%%%%find at which time point, fly on right side,RTP = right time point%%%
preRightRow = tarow-1;
PRTP = train1(preRightRow(2:end)); %%%%%%%%%the last time point before  RTP, PRTP = pre right time point%%%%
ta = sum(RTP-PRTP); %%%%%the total time when fly on the right side, i.e. non punishment time%%
tb = 120 - ta; %%%%%%tb means punishment time,
...
tarow = find(train2Val<0);  %%%%%%%%%% find which row that trace value<0, i.e when turning right%%%
RTP = train2(tarow(2:end)); %%%%%%find at which time point, fly on right side,RTP = right time point%%%
preRightRow = tarow-1;
PRTP = train2(preRightRow(2:end));%%%%%%%%%the last time point before  RTP, PRTP = pre right time point%%%%
ta = sum(RTP-PRTP); %%%%%the total time when fly on the right side, i.e. non punishment time%%
tb = 120 - ta; %%%%%%tb means punishment time,
...
tarow = find(test1Val<0);  %%%%%%%%%% find which row that trace value<0, i.e when turning right%%%
RTP = test1(tarow(2:end)); %%%%%%find at which time point, fly on right side,RTP = right time point%%%
preRightRow = tarow-1;
PRTP = test1(preRightRow(2:end));%%%%%%%%%the last time point before  RTP, PRTP = pre right time point%%%%
ta = sum(RTP-PRTP); %%%%%the total time when fly on the right side, i.e. non punishment time%%
tb = 120 - ta; %%%%%%tb means punishment time,
test1PI = (ta-tb)/(ta+tb)
